...
public class AutomationController {
  ...
  @PreAuthorize("hasAuthority('system.configure')")
  public AutomationRule version(@PathVariable("id") UUID id, @Valid @RequestBody AutomationRule rule, Principal actor) {
  ...
  @PreAuthorize("hasAuthority('system.configure')")
  public AutomationRule enabled(@PathVariable("id") UUID id, @RequestParam("value") boolean value, Principal actor) {
  ...
  public AutomationDecision simulate(@PathVariable("assetId") UUID assetId) {
  ...
  public AutomationDecision evaluate(@PathVariable("assetId") UUID assetId) {
    return runtime.evaluate(assetId, false, "MANUAL_EVALUATION");
  }

  @PostMapping("/assets/{assetId}/parking")
  @PreAuthorize("hasAuthority('dispatch.override')")
  public AutomationDecision parking(@PathVariable("assetId") UUID assetId) {
    return runtime.evaluate(assetId, false, "DISPATCHER_AUTO_PARK");
  }

  @PostMapping("/assets/{assetId}/fueling")
  @PreAuthorize("hasAuthority('dispatch.override')")
  public AutomationDecision fueling(@PathVariable("assetId") UUID assetId) {
  ...
  public void release(@PathVariable("id") UUID id, Principal principal) { runtime.reservations().release(id); runtime.audit(principal.getName(), "RESERVATION_RELEASED", id.toString(), Map.of()); }
  ...
  @GetMapping("/candidates/parking/{assetId}") public Object parkingCandidates(@PathVariable("assetId") UUID assetId) {
    return candidates(runtime.evaluate(assetId, true, "PARKING_CANDIDATE_QUERY"), "P-");
  }
  @GetMapping("/candidates/fueling/{assetId}") public Object fuelingCandidates(@PathVariable("assetId") UUID assetId) {
  ...
  @PreAuthorize("hasAuthority('dispatch.override')")
  public Map<String, Object> approve(@PathVariable("decisionId") UUID decisionId, Principal principal) {
