...
export interface MapHandle { addVehicles(vehicles:MapVehicle[]):void; destroy():void; }
...
class LeafletRasterProvider implements MapProvider {
  ...
  async mount(element:HTMLElement,c:MapConfiguration):Promise<MapHandle>{
    ...
    return {addVehicles(vehicles){vehicles.forEach(v=>{const icon=L.divIcon({className:'vehicle-pin',html:`<span class="${v.stale?'stale':''}">▰</span>`,iconSize:[28,28]});L.marker([v.latitude,v.longitude],{icon,title:v.name}).bindPopup(`<b>${v.name}</b><br>${v.status}<br>${v.stale?'Location stale':'Live position'}`).addTo(cluster)})},destroy(){map.remove()}};
  }
}
...
class GoogleProvider implements MapProvider {
  ...
  async mount(element:HTMLElement,c:MapConfiguration):Promise<MapHandle>{
    ...
    const googleApi=(window as unknown as {google:{maps:{Map:new(e:HTMLElement,o:object)=>unknown;Marker:new(o:object)=>unknown}}}).google;
    ...
    return {addVehicles(vehicles){vehicles.forEach(v=>new googleApi.maps.Marker({map,position:{lat:v.latitude,lng:v.longitude},title:`${v.name} · ${v.status}`}))},destroy(){element.replaceChildren()}};
  }
}
