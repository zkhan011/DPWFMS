...
export interface MapHandle { setVehicles(vehicles:MapVehicle[]):void; setVehiclesVisible(visible:boolean):void; destroy():void; }
...
class GoogleProvider implements MapProvider {
  ...
  async mount(element:HTMLElement,c:MapConfiguration):Promise<MapHandle>{
    ...
    return {setVehicles(vehicles){markers.forEach(marker=>marker.setMap(null));markers.length=0;vehicles.forEach(v=>markers.push(new googleApi.maps.Marker({map,position:{lat:v.latitude,lng:v.longitude},title:`${v.name} · ${v.status}`})))},setVehiclesVisible(visible){markers.forEach(marker=>marker.setMap(visible?map:null))},destroy(){markers.forEach(marker=>marker.setMap(null));element.replaceChildren()}};
  }
}
